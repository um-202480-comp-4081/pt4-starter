...
RSpec.describe 'tracks/index.html.erb' do
  before do
    assign :tracks, build_stubbed_list(:track, 2)
    ...
    allow(view).to receive(:new_track_path).and_call_original
    allow(view).to receive(:edit_track_path).and_call_original
  ...
  it 'uses appropriate route helper(s)' do
    expect(view).to have_received(:track_path).exactly(4).times
    expect(view).to have_received(:new_track_path).exactly(1).times
    expect(view).to have_received(:edit_track_path).exactly(2).times
  end
